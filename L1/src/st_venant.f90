...
PROGRAM ST_VENANT
    ...
    ELSE
        ...
        CALL WRITE_NC(vx_t(1:Nx), vy_t(1:Ny), vtime, h(1:Nx, 1:Ny, :), 'data/h_f2.nc', 'h')

        IF (l_write_uv) THEN
            CALL WRITE_NC(vx_u(0:Nx), vy_t(1:Ny), vtime, u(0:Nx, 1:Ny, :), 'data/u_f2.nc', 'u')
            CALL WRITE_NC(vx_t(1:Nx), vy_v(0:Ny), vtime, v(1:Nx, 0:Ny, :), 'data/v_f2.nc', 'v')
        END IF
...
CONTAINS
    ...
    SUBROUTINE COMPUTE_ENERGY()
        ...
        OPEN (11, file='data/energy_f2.dat', status='replace')
        DO jt = 1, nb_save
